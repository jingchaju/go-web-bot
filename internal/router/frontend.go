...
func installFrontendRoutes(r *gin.Engine, cfg config.Config) {
	...
	r.NoRoute(func(c *gin.Context) {
		...
		if isReservedBackendPath(c.Request.URL.Path, cfg.AdminRoutePrefix) {
			c.JSON(http.StatusNotFound, gin.H{"message": "not found"})
			return
		}
		if serveFrontendDistFile(c, dist) {
			return
		}
		c.File(indexPath)
	})
...
func isReservedBackendPath(path, adminPrefix string) bool {
...
func serveFrontendDistFile(c *gin.Context, dist string) bool {
